...
enum TokenKind {
	EOF("<EOF>"), ABSTRACT("abstract"), BOOLEAN("boolean"), BREAK("break"), BYTE("byte"), CASE("case"), CATCH("catch"), 
	CHAR("char"), CLASS("class"), CONST("const"), CONTINUE("continue"), DEFAULT("default"), DO("do"), DOUBLE("double"), ELSE("else"),
	EXTENDS("extends"), FINAL("final"), FINALLY("finally"), FLOAT("float"), FOR("for"), GOTO("goto"), FALSE("false"), IF("if"), IMPLEMENTS("implements"),
	IMPORT("import"), INSTANCEOF("instanceof"), INT("int"), INTERFACE("interface"), LONG("long"), NATIVE("native"), NEW("new"),
	NULL("null"), PACKAGE("package"), PRIVATE("private"), PROTECTED("protected"), PUBLIC("public"), RETURN("return"),
	SHORT("short"), STATIC("static"), STRICTFP("strictfp"), SUPER("super"), SWITCH("switch"), SYNCRONIZED("synchronized"), THIS("this"), 
	THROW("throw"), THROWS("throws"), TRANSIENT("transient"), TRY("try"), TRUE("true"), VOID("void"), VOLATILE("volatile"), WHILE("while"), 
	TERN("?"), PLUS("+"),	ASSIGN("="), DEC("--"), EQUAL("=="), GT(">"), LT("<"), INC("++"), LAND("&&"),
	LE("<="), GE(">="),	LNOT("!"), LNOT_EQUAL("!="), MINUS("-"), PLUS_ASSIGN("+="), MINUS_ASSIGN("-="),
	STAR("*"), STAR_ASSIGN("*="), DIV("/"),	DIV_ASSIGN("/="), REM("%"), REM_ASSIGN("%="), AND("&"),
	AND_ASSIGN("&="), XOR("^"),	XOR_ASSIGN("^="), OR("|"), OR_ASSIGN("|="),	LOR("||"),
	LSHIFT("<<"), LSHIFT_ASSIGN("<<="),	RSHIFT(">>"), RSHIFT_ASSIGN(">>="),	LRSHIFT(">>>"),	
	LRSHIFT_ASSIGN(">>>="),	LLSHIFT("<<<"), NOT("~"),
	LPAREN("("), RPAREN(")"), LCURLY("{"), RCURLY("}"), LBRACK("["), RBRACK("]"), SEMI(";"), COLON(":"), COMMA(","), DOT("."),
	IDENTIFIER("<IDENTIFIER>"), INT_LITERAL("<INT_LITERAL>"), DOUBLE_LITERAL("<DOUBLE_LITERAL>"), CHAR_LITERAL("<CHAR_LITERAL>"), STRING_LITERAL("<STRING_LITERAL>");
