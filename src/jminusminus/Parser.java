...
public class Parser {
	 ...
	 * </pre>
	 * 
	 * @return an AST for a bitwiseExpressionOne.
	 */

	private JExpression bitwiseExpressionFour() {
		int line = scanner.token().line();
		boolean more = true;
		JExpression lhs = bitwiseExpressionThree();
		while (more) {
			if (have(OR)) {
				lhs = new JOrOp(line, lhs, bitwiseExpressionThree());
			} else {
				more = false;
			}
		}
	...
	private JExpression bitwiseExpressionThree() {
		int line = scanner.token().line();
		boolean more = true;
		JExpression lhs = bitwiseExpressionTwo();
		while (more) {
			if (have(XOR)) {
				lhs = new JXorOp(line, lhs, bitwiseExpressionTwo());
			} else {
				more = false;
			}
		}
	...
	private JExpression relationalExpression() {
		int line = scanner.token().line();
		JExpression lhs = bitwiseExpressionOne();
		if (have(GT)) {
			return new JGreaterThanOp(line, lhs, bitwiseExpressionOne());
		} else if (have(LE)) {
			return new JLessEqualOp(line, lhs, bitwiseExpressionOne());
	...
	private JExpression bitwiseExpressionOne() {
		...
		while (more) {
			if (have(LSHIFT)) {
				...
				lhs = new JLRShiftOp(line, lhs, additiveExpression());
			} else {
				more = false;
			}
		}
